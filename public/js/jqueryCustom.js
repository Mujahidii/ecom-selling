jQuery(document).ready(function() {
	...
	jQuery('.ctgList li > a').on("click", function(e){
		jQuery(this).next('.ctChildList').parent('li').toggleClass('hover');
		jQuery(this).next('.ctChildList').toggleClass('hover');
		e.stopPropagation();
		e.preventDefault();
	});
	...
	function initAddClass() {
		...
		jQuery('.cartOpener').click(function(){
			jQuery("body").addClass("cartActive");
		});

		jQuery('.sideNav .sideClose').click(function(){
		   jQuery('body').removeClass('cartActive');
		});

		jQuery('.recLink').on( "click", function(e){
			e.preventDefault();
			jQuery("body").toggleClass("recActive");
		});

		jQuery(".ctgList li a.hasHover").hover(
			function () {
	...
	function initStickyScrollBlock() {
		jQuery('.sticky-wrap-headerFixer').stickyScrollBlock({
			setBoxHeight: true,
	...
	function initSlickCarousel() {
		...
		jQuery('.recVieSlider, .relProdSlider').slick({
			slidesToShow: 6,
			slidesToScroll: 1,
			dots: false,
			arrows: true,
		...
		jQuery('.quickReiwProductSlider').slick({
			slidesToShow: 1,
			slidesToScroll: 1,
			dots: true,
			autoplay: true,
		...
		jQuery('.featurePostSlider').slick({
			slidesToShow: 1,
			slidesToScroll: 1,
			dots: false,
			arrows: true,
		...
		jQuery('.detailsImageSlider2').slick({
			slidesToScroll: 1,
			rows: 0,
			arrows: false,
			fade: true,
			adaptiveHeight: true,
			asNavFor: '.verticledisSwitcherSlider'
		});

		jQuery('.verticledisSwitcherSlider').slick({
			slidesToScroll: 1,
			rows: 0,
			slidesToShow: 4,
			arrows: false,
...
jQuery('<div class="quantity-nav"><div class="quantity-button quantity-plus d-flex align-items-center justify-content-center"><i class="fas fa-plus"><span class="sr-only">icon</span></i></div><div class="quantity-button quantity-minus d-flex align-items-center justify-content-center"><i class="fas fa-minus"><span class="sr-only">icon</span></i></div></div>').insertAfter('.quantity input');
...
jQuery(window).on('load', function() {
	...
	function onHashchange2() {
		...
		$('.filterNav ').each(function(i, obj) {
	  		var hashid=$(this).attr("id");
	  		var hashFilter=$("#"+hashid+" > li .is-checked").attr("data-filter");
	  		var hashtype=$("#"+hashid+" > li .is-checked").attr("data-type");
	  		var chechash="filter="+hashFilter+"&type="+hashtype;
	  		
			chechash=chechash.split("&");
			chechash=chechash[1];
			chechash=chechash.split("=");
			chechash=chechash[1];
				
			var $grid = $('#gridFilterWrap'+chechash);
			var $filterButtonGroup2 = $('#filterNav'+chechash);
			isIsotopeInit = true;
			// filter isotope
			$grid.isotope({
			...
			if ( hashFilter ) {
			  $filterButtonGroup2.find('.is-checked').removeClass('is-checked');
			  $filterButtonGroup2.find('[data-filter="' + hashFilter + '"]').addClass('is-checked');
			}	
		});
	}
	...
	var $filterButtonGroup = $('.filterNav');
	$filterButtonGroup.on( 'click', '.filterBtn', function() {
	  var filterAttr = $( this ).attr('data-filter');
	  var filterType = $( this ).attr('data-type');
	// set filter in hash
	location.hash = 'filter=' + encodeURIComponent( filterAttr )+'&type='+filterType;
	});
	...
	function onHashchange() {
		...
		if ( (!hashFilter && isIsotopeInit) ||(hashFilter==null)) {
		   return;
	   }


	   // init Isotope
	   	var chechash = location.hash;
		chechash=chechash.split("&");
		chechash=chechash[1];
		chechash=chechash.split("=");
		chechash=chechash[1];
